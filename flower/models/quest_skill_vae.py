...
class SkillVAE(nn.Module):
    def __init__(self,
                 ...
                 codebook_dim,
                 codebook_size,
                 quantize_rotation="none",
                 quantize_rotation_seed=0,
                 ):
        super().__init__()
        ...
        self.quantize_rotation = quantize_rotation
        self.quantize_rotation_seed = quantize_rotation_seed
        ...
        self._setup_quantize_rotation(encoder_dim, quantize_rotation, quantize_rotation_seed)
    ...
    def _setup_quantize_rotation(self, dim, quantize_rotation, seed):
        if quantize_rotation is None:
            quantize_rotation = "none"
        self.quantize_rotation_location = "none"
        self.use_quantize_rotation = quantize_rotation != "none"
        if not self.use_quantize_rotation:
            return

        pre_project_modes = ("random_orthogonal", "orthogonal")
        fsq_projected_modes = (
            "fsq_projected_random_orthogonal",
            "fsq_projected_orthogonal",
            "projected_random_orthogonal",
            "projected_orthogonal",
        )
        if quantize_rotation in pre_project_modes:
            rotation_dim = dim
            self.quantize_rotation_location = "pre_project"
        elif quantize_rotation in fsq_projected_modes:
            if self.vq_type != "fsq":
                raise ValueError("FSQ projected rotation is only supported when vq_type='fsq'")
            rotation_dim = self.vq.codebook_dim
            self.quantize_rotation_location = "fsq_projected"
        else:
            raise ValueError(f"Unknown quantize_rotation: {quantize_rotation}")

        generator = torch.Generator(device="cpu")
        generator.manual_seed(seed)
        matrix = torch.randn(rotation_dim, rotation_dim, generator=generator)
        q, r = torch.linalg.qr(matrix)
        signs = torch.sign(torch.diag(r))
        signs[signs == 0] = 1
        q = q * signs.unsqueeze(0)
        self.register_buffer("quantize_rotation_matrix", q)

    def _rotate_for_quantize(self, x):
        if self.quantize_rotation_location != "pre_project":
            return x
        rotation = self.quantize_rotation_matrix.to(device=x.device, dtype=x.dtype)
        return torch.matmul(x, rotation)

    def _inverse_rotate_from_quantize(self, x):
        if self.quantize_rotation_location != "pre_project":
            return x
        rotation = self.quantize_rotation_matrix.to(device=x.device, dtype=x.dtype)
        return torch.matmul(x, rotation.transpose(0, 1))

    def _rotate_fsq_projected_for_quantize(self, x):
        if self.quantize_rotation_location != "fsq_projected":
            return x
        rotation = self.quantize_rotation_matrix.to(device=x.device, dtype=x.dtype)
        return torch.matmul(x, rotation)

    def _inverse_rotate_fsq_projected_from_quantize(self, x):
        if self.quantize_rotation_location != "fsq_projected":
            return x
        rotation = self.quantize_rotation_matrix.to(device=x.device, dtype=x.dtype)
        return torch.matmul(x, rotation.transpose(0, 1))
    ...
    def quantize(self, z):
        z_quant = self._rotate_for_quantize(z)
        if self.vq_type == 'vq':
            codes, indices, commitment_loss = self.vq(z_quant)
            pp = torch.tensor(torch.unique(indices).shape[0] / self.vq.codebook_size, device=z.device)
        else:
            if self.quantize_rotation_location == "fsq_projected":
                codes, indices = self._quantize_fsq_projected_rotation(z_quant)
            else:
                codes, indices = self.vq(z_quant)
            commitment_loss = torch.tensor([0.0], device=z.device)
            pp = torch.tensor(torch.unique(indices).shape[0] / self.vq.codebook_size, device=z.device)
        codes = self._inverse_rotate_from_quantize(codes)
    ...
    def _quantize_fsq_projected_rotation(self, z):
        orig_dtype = z.dtype
        z = self.vq.project_in(z)
        z = z.reshape(*z.shape[:-1], self.vq.num_codebooks, self.vq.codebook_dim)

        if z.dtype not in self.vq.allowed_dtypes:
            z = z.float()

        z = self._rotate_fsq_projected_for_quantize(z)
        codes = self.vq.quantize(z)
        indices = self.vq.codes_to_indices(codes)
        codes = self._inverse_rotate_fsq_projected_from_quantize(codes)
        codes = codes.reshape(*codes.shape[:-2], self.vq.effective_codebook_dim)

        if codes.dtype != orig_dtype:
            codes = codes.type(orig_dtype)

        codes = self.vq.project_out(codes)
        if not self.vq.keep_num_codebooks_dim:
    ...
    def decode_actions(self, indices):
        if self.vq_type == 'fsq':
            if self.quantize_rotation_location == "fsq_projected":
                codes = self.vq._indices_to_codes(indices)
                codes = self._inverse_rotate_fsq_projected_from_quantize(codes)
                if self.vq.keep_num_codebooks_dim:
                    codes = codes.reshape(*codes.shape[:-2], self.vq.effective_codebook_dim)
                codes = self.vq.project_out(codes)
            else:
