...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-6,
        help="Minimum std used for per-dimension action normalization.",
    )
...
def compute_action_statistics(
    ...
    *,
    ...
    for record in tqdm(records, desc="Computing global action statistics"):
    ...
    if not actions:
        raise ValueError("Could not compute action statistics because no action steps were found.")

    action_array = np.asarray(actions, dtype=np.float32)
    action_mean = action_array.mean(axis=0).astype(np.float32)
    action_std = action_array.std(axis=0).astype(np.float32)
    action_std = np.maximum(action_std, epsilon).astype(np.float32)

    return {
        "mean": action_mean,
        "std": action_std,
        "min": action_array.min(axis=0).astype(np.float32),
        "max": action_array.max(axis=0).astype(np.float32),
        "count": np.asarray([action_array.shape[0]], dtype=np.int64),
    }


def normalize_action(action: np.ndarray, action_stats: Dict[str, np.ndarray]) -> np.ndarray:
    return ((action - action_stats["mean"]) / action_stats["std"]).astype(np.float32)
...
def save_timestep_npz(
    ...
    *,
    ...
    rel_actions = normalize_action(rel_actions, action_stats)
...
def write_split(
    ...
    *,
    ...
    summary = {
        ...
        "action_mean": action_stats["mean"].tolist(),
        "action_std": action_stats["std"].tolist(),
...
def main() -> None:
    ...
    action_stats = compute_action_statistics(
        ...
        epsilon=args.action_norm_epsilon,
    )
    LOGGER.info("Global action mean: %s", action_stats["mean"])
    LOGGER.info("Global action std: %s", action_stats["std"])

    np.savez(
        args.output_root / "action_statistics.npz",
    ...
    overall_summary = {
        ...
        "include_right_ft": args.include_right_ft,
        "action_statistics_path": str(args.output_root / "action_statistics.npz"),
        "action_mean": action_stats["mean"].tolist(),
        "action_std": action_stats["std"].tolist(),
        "action_min": action_stats["min"].tolist(),
        "action_max": action_stats["max"].tolist(),
        "action_count": int(action_stats["count"][0]),
