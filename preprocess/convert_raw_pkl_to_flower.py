"""
Convert raw robot pickle data into a FLOWER / CALVIN-style disk dataset.
...
The script is intentionally conservative:
    - It keeps only right-arm state / action by default.
    - It emits dummy language embeddings because FLOWER training in this repo
      uses lang_text directly during observation encoding.
    - It skips short episodes by default because the provided FLOWER config
      expects sequences longer than the action chunk size.
"""
...
LOGGER = logging.getLogger("convert_raw_pkl_to_flower")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Raw pickle file or directory containing pickle files.",
    )
    parser.add_argument(
        "--output_root",
        required=True,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Validation split ratio at episode level.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--task_name",
        default="custom_task",
        help="Task label stored in auto_lang_ann.npy when no per-episode task is provided.",
    )
    parser.add_argument(
        "--default_instruction",
        default=None,
        help="Fallback instruction used when an episode-specific instruction is not found.",
    )
    parser.add_argument(
        "--instructions_json",
        type=Path,
        ...
        help=(
            ...
            "Supported keys: source_name, source_name#episode_idx, or global episode index string."
        ),
    )
    parser.add_argument(
        "--lang_folder",
...
def load_episode_records(input_path: Path, pkl_glob: str) -> List[EpisodeRecord]:
    if input_path.is_file():
        payload = load_pickle(input_path)
        episodes = extract_episodes_from_object(payload)
        return [
            EpisodeRecord(steps=steps, source_name=input_path.stem, source_episode_index=idx)
    ...
    pkl_files = sorted(input_path.glob(pkl_glob))
    if not pkl_files:
        raise FileNotFoundError(f"No pickle files matched {pkl_glob} in {input_path}")

    first_payload = load_pickle(pkl_files[0])
    if is_single_step_transition(first_payload):
        LOGGER.info("Detected directory of step-level pickle files.")
        ...
        return [
            EpisodeRecord(steps=steps, source_name="step_stream", source_episode_index=idx)
            for idx, steps in enumerate(episodes)
        ]

    records: List[EpisodeRecord] = []
    for path in tqdm(pkl_files, desc="Loading pickle files"):
        payload = load_pickle(path)
        episodes = extract_episodes_from_object(payload)
        for idx, steps in enumerate(episodes):
            records.append(EpisodeRecord(steps=steps, source_name=path.stem, source_episode_index=idx))
    return records
...
def resolve_instruction(
    ...
    if mapping is None:
        return default_instruction, default_task_name
    ...
    if isinstance(value, str):
        return value, default_task_name

    if isinstance(value, dict):
        instruction = value.get("instruction", default_instruction)
        task_name = value.get("task", default_task_name)
...
def split_records(records: Sequence[EpisodeRecord], val_ratio: float, seed: int) -> Tuple[List[Tuple[int, EpisodeRecord]], List[Tuple[int, EpisodeRecord]]]:
    indexed_records = list(enumerate(records))
    if not indexed_records:
        return [], []

    shuffled = indexed_records[:]
    ...
    val_slice = shuffled[:val_count]
    train_slice = shuffled[val_count:]
...
def main() -> None:
    ...
    records = load_episode_records(args.input, args.pkl_glob)
