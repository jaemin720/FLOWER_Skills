...
def smooth_history_sequence(history: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    ...
    if len(original_shape) < 2:
        raise ValueError(f"F/T history must have at least 2 dimensions, got shape {original_shape}")

    def apply_filter(sequence: np.ndarray) -> np.ndarray:
        if args.ft_history_filter == "ema":
            return causal_ema_sequence(sequence, args.ft_history_ema_alpha)
        if args.ft_history_filter == "butterworth":
            return causal_butterworth_sequence(
                sequence,
    ...
    if history.ndim == 2:
        return apply_filter(history).astype(np.float32)

    # Shape is usually (episode_steps, history_len, ft_dim). Each stored history
    # window is already causal, so smooth along the window time axis only. Do not
    # flatten episode_steps and history_len together; adjacent windows overlap and
    # flattening them makes the filter see time jump backwards at every step.
    window_count = int(np.prod(original_shape[:-2]))
    history_windows = history.reshape(window_count, original_shape[-2], original_shape[-1])
    smoothed = np.stack([apply_filter(window) for window in history_windows], axis=0)
...
def process_ft_history_observations(
    ...
    *,
    ...
    raw_by_key: Dict[str, List[np.ndarray]] = {key: [] for key in history_keys}
    for record in records:
        for key in history_keys:
            raw_by_key[key].append(extract_history_sequence(record, key))
    ...
    if args.normalize_ft_history:
        for key, sequences in raw_by_key.items():
            if key in stats:
                continue
            if "default" in stats:
...
def save_timestep_npz(
    ...
    *,
    ...
    # F/T history is smoothed/normalized once per split before writing.
